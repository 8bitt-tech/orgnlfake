...
export async function POST(req: Request) {
  try {
    ...
    // 3. Email the admin & booking team
    const emailHtml = `
    ...
    const { success, error } = await sendMail({
      from: 'Portfolio Access System <booking@orgnlfake.agency>',
    ...
    if (!success) {
      console.error("sendMail failed in /api/request-access:", error);
      // We don't return 500 here because the database insert succeeded, and they should still see success on the frontend.
